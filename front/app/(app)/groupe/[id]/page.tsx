...
import { getGroupeById, getGroupeSemaine } from '../../../api/requests/requests_groupes'
...
export default function AppGroupe({ params }: { params: { id: string } }) {
  ...
  // actualGroupe
  const [actualGroupe, setActualGroupe] = useState({} as Groupes)
  // FavorisGroupe
  ...
  useEffect(() => {
    ...
    // Récupère le groupe
    getGroupeById(params.id).then((res) => {
        setActualGroupe(res)
    })
  }, [])
